    ...
    public void run() {
        ...
        for (ResourceLocation blockId : Registry.BLOCK.keySet()) {
            ...
            if (!block.defaultBlockState().getValues().isEmpty()) {
                ...
                block.defaultBlockState().getValues().entrySet().stream().sorted(Comparator.comparing(e -> e.getKey().getName())).forEach(e -> bits.add(e.getKey().getName() + "=" + e.getValue().toString().toLowerCase()));
