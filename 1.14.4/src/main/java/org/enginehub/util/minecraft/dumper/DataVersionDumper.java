    ...
    public void run() {
        ...
        for (Identifier blockId : Registry.BLOCK.getIds()) {
            ...
            if (!block.getDefaultState().getEntries().isEmpty()) {
                ...
                block.getDefaultState().getEntries().entrySet().stream().sorted(Comparator.comparing(e -> e.getKey().getName())).forEach(e -> bits.add(e.getKey().getName() + "=" + e.getValue().toString().toLowerCase()));
        ...
        // BlockTags
        Map<String, List<String>> blockTags = getTags(BlockTags.getContainer(), Registry.BLOCK);

        // ItemTags
        Map<String, List<String>> itemTags = getTags(ItemTags.getContainer(), Registry.ITEM);

        // EntityTags
        Map<String, List<String>> entityTags = getTags(EntityTypeTags.getContainer(), Registry.ENTITY_TYPE);
